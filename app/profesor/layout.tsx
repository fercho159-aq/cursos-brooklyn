...
export default function ProfesorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--light)', display: 'flex' }}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} style={{
                ...
                padding: sidebarOpen ? '12px 15px' : '12px', marginBottom: '5px',
                ...
                <FontAwesomeIcon icon={item.icon} style={{ width: '20px', color: active ? 'var(--primary)' : 'var(--gray)' }} />
                {sidebarOpen && <span>{item.label}</span>}
                {sidebarOpen && active && <FontAwesomeIcon icon={faChevronRight} style={{ marginLeft: 'auto', fontSize: '0.75rem' }} />}
              </Link>
            )
          })}
        </nav>
      ...
      {/* Main Content */}
      <main className="profesor-main-content" style={{ flex: 1, marginLeft: sidebarOpen ? '250px' : '70px', transition: 'margin-left 0.3s ease', minHeight: '100vh' }}>
        {children}
      </main>
      ...
      <style jsx global>{`
        @media (max-width: 768px) {
          ...
          .profesor-mobile-header { display: flex !important; }
